# 2021年“天翼杯”—— Crypto —— MyHash

题目见：`TryHash.py`

一道水题，之前是没时间看以为很难，一看被人wirteup发现水得1批。

题目说是hash，但是实际上是一个TEA加密，一种分组密码算法。

这道题目水的一点在于密钥是我们给出的，我们只需要把明文算出来就行，设置key = 0，然后加密用的msg很快就能算出来。然后就可以对oracle的挑战进行加密，然后得到flag。然后对着加密算法写解密算法仔细看看也能很快写出来。

参考代码：见`ZMJ_solution.py`，代码不长就贴一下
```python
from pwn import *
...
代码参考wp：https://zhuanlan.zhihu.com/p/413319231